...
const Auth = ({ className }: ComponentProps<"div">) => {
...
const AuthForm = ({ onSubmit, children, className }: AuthFormProps) => (
...
const AuthSuccess = ({ message }: AuthSuccessProps) => {
    if (!message) return null
    return (
        <div
            data-slot="auth-success"
            className="animate-in mb-6 rounded-lg border border-emerald-500/20 bg-emerald-500/10 p-4 text-sm text-emerald-600 dark:text-emerald-400"
        >
            {message}
        </div>
    )
...
const AuthSignUp = ({ onSignIn }: AuthSignUpProps) => {
    ...
    return (
        ...
        >
            ...
            <AuthError message={formState.error} />
            <AuthSuccess message={formState.success} />
